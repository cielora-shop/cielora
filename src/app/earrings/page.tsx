...
function EarringsContent() {
  ...
  const topFilters = navbarTabs.find(t => t.href === '/earrings')?.groupFilters || [];

  const bottomFilters = [
    "CATEGORY", "PRICE", "SIZE", "PLATING", "COMPONENT", "LEATHER", "COLOR"
  ];

  const categoryProducts = dbProducts.filter(p => p.category?.toLowerCase().includes("earring"));
  ...
  if (loading) {
    return (
      <div className="w-full bg-white min-h-screen flex items-center justify-center text-gray-500 font-medium">
        {isEs ? "Cargando..." : "{isEs ? "Cargando..." : "{isEs ? "Cargando..." : "Loading earrings..."}"}"}
      </div>
    );
  }
    ...
    <div className="w-full bg-white min-h-screen">
      <div className="px-4 md:px-12 xl:px-24 pt-4 md:pt-8 pb-12">
        ...
        <div className="text-[10px] text-gray-500 mb-[12px]">
          <Link href="/" className="hover:underline">{isEs ? "Inicio" : "Home"}</Link> / <span className="text-black">{isEs ? "Pendientes" : "Earrings"}</span>
        </div>

        {/* Title */}
        <h1 className="text-[24px] font-semibold text-black pb-[4px] mb-4">{isEs ? "Pendientes para mujer" : "Earrings for women"}</h1>
