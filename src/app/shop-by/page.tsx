  ...
  const filteredProducts = dbProducts.filter(p => {
    ...
    if (selectedTopFilter) {
      const getKeywords = (filter: string) => {
        let base = filter.toLowerCase().replace(" jewelry", "").replace(" jewerly", "");
        if (base === "accesories") return ["accessory", "accessories", "accesories", "accesory"];
        if (base === "women's") return ["women", "woman"];
        if (base === "men's") return ["men", "man"];
        if (base === "dragonfly") return ["dragonfly", "dragonflies"];
        if (base === "best sellers") return ["best seller", "best sellers"];
        return [base, base.replace(/s$/, ''), base.replace(/es$/, ''), base.replace(/ies$/, 'y')].filter(Boolean);
      };
    ...
    <div className="w-full bg-white min-h-screen">
      <div className="px-4 md:px-12 xl:px-24 pt-4 md:pt-8 pb-12">
        ...
        <div className="text-[10px] text-gray-500 mb-[12px]">
          <Link href="/" className="hover:underline">{isEs ? "Inicio" : "Home"}</Link> / <span className="text-black">{isEs ? "Comprar por" : "Shop by"}</span>
        </div>

        {/* Title */}
        <h1 className="text-[24px] font-semibold text-black pb-[4px] mb-4">{isEs ? "Comprar por" : "Shop by"}</h1>
