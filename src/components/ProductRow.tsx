  ...
  return (
    <section className={`${className} px-4 sm:px-6 lg:px-8 w-full`}>
      ...
      <div 
        ...
        className="flex overflow-x-auto snap-x snap-mandatory [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] space-x-1"
      >
        {rowProducts.map((product) => (
          <div key={product.id} className="w-[50%] md:w-[25%] flex-shrink-0 snap-start pr-1">
            <ProductCard 
              id={product.id}
              title={product.title}
              price={product.price}
